...

{
	...
	for (int i = 0; i < maxParticulas_; i++){
		particulas_.push_back(new PeParticula(0.2, 0.002,vida, origen_, origen_));
	}
...
bool PeSPExplosion::SimulaPasoParticula(PeParticula*p, float deltaTime){
	...
	else
	{
		...
		p->vel_ = vec3Add(p->vel_,vec3Multiply(vec3Divide(vec3Multiply(p->fuerzas_, p->masa_), 1),deltaTime));
